...
import { saveMunFormConfig, getMunFormConfig } from "./actions";
...
const DEFAULT_FIELDS: FormField[] = [
  ...
  { id: "school", type: "text", label: "School / Institution", placeholder: "Enter your school or college", required: true, locked: true },
  { id: "grade", type: "text", label: "Grade / Year", placeholder: "e.g. 11th, 1st Year", required: false, locked: true },
  { id: "experience", type: "dropdown", label: "MUN Experience", required: true, locked: true, options: "NONE,BEGINNER,INTERMEDIATE,ADVANCED" },
  { id: "committee_pref_1", type: "dropdown", label: "1st Committee Preference", required: true, locked: true, description: "Auto-populated from your committees" },
  { id: "committee_pref_2", type: "dropdown", label: "2nd Committee Preference", required: false, locked: true, description: "Optional" },
  { id: "committee_pref_3", type: "dropdown", label: "3rd Committee Preference", required: false, locked: true, description: "Optional" },
];
...
export default function FormBuilderPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [formSettings, setFormSettings] = useState({ isClosed: false, closedMessage: "", waitlistEnabled: false });
  ...
  useEffect(() => {
    ...
    async function loadConfig() {
      try {
        const savedData = await getMunFormConfig(resolvedParams.id);
        const localSaved = localStorage.getItem(`freo_mun_builder_progress_${resolvedParams.id}`);
        ...
        if (savedData) {
          if (savedData.custom_form_schema && Array.isArray(savedData.custom_form_schema) && savedData.custom_form_schema.length > 0) {
            loadedFields = savedData.custom_form_schema as FormField[];
            hasCloudData = true;
          }
        }
        ...
        // Ensure all locked default fields are present (handles old configs with fewer defaults)
        const existingIds = new Set(loadedFields.map(f => f.id));
        const missingDefaults = DEFAULT_FIELDS.filter(df => df.locked && !existingIds.has(df.id));
        if (missingDefaults.length > 0) {
          // Insert missing defaults after the last locked field
          const lastLockedIdx = loadedFields.reduce((acc, f, i) => f.locked ? i : acc, -1);
          loadedFields.splice(lastLockedIdx + 1, 0, ...missingDefaults);
        }
  ...
  useEffect(() => {
    if (!isMounted) return;
    localStorage.setItem(`freo_mun_builder_progress_${resolvedParams.id}`, JSON.stringify({
      fields
    }));
  }, [fields, isMounted, resolvedParams.id]);
  ...
  useEffect(() => {
    ...
    if (currentFieldsString === lastSavedFields) return; // No changes to save

    const timer = setTimeout(async () => {
      setIsSaving(true);
      try {
        await saveMunFormConfig(resolvedParams.id, fields);
  ...
  }, [fields, isMounted, resolvedParams.id, lastSavedFields]);
      ...
      <div className={`max-w-7xl mx-auto space-y-8 px-4 sm:px-6 lg:px-8 pb-12`}>
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <Link href={`/mun/dashboard/conference/${resolvedParams.id}`}>
          ...
          <div className="flex flex-wrap gap-3 items-center">
            ...
            {isFirstTimeBuilder && (
              <Link href={`/mun/dashboard/conference/${resolvedParams.id}/delegates`}>
